...
def _router_bias_update_method(config) -> str:
    ...
    if load_balancing_type == "quantile_balancing" or (
        isinstance(load_balancing_type, (list, tuple))
        and "quantile_balancing" in load_balancing_type
    ):
        return "quantile"
...
def _router_bias_update_fn(config):
    method = _router_bias_update_method(config)
    if method == "none":
        return None
    if method == "quantile":
        update_fn = getattr(
            finalize_model_grads_module, "_update_router_expert_bias_with_quantile", None
        )
        if update_fn is None:
            raise RuntimeError(
                "Quantile balancing requested, but this MCore checkout does not expose the "
                "global-batch quantile router-bias finalizer."
            )
        return update_fn
    return finalize_model_grads_module._update_router_expert_bias


def _verify_router_bias_metadata(config, layers: torch.nn.ModuleList) -> dict[int, tuple[int, ...]]:
    routers = _router_bias_modules(layers)
    method = _router_bias_update_method(config)
    use_quantile_balancing = method == "quantile"
    if method == "none":
        if routers:
    ...
    _router_bias_update_fn(config)
    te_qb_api = bool(getattr(te_extensions, "fused_topk_with_score_function_supports_qb", False))
    router_fusion = bool(getattr(config, "moe_router_fusion", False))
    pointers = {}
    for router in routers:
        pointer_values = [router.expert_bias.data_ptr()]
        if use_quantile_balancing:
            histogram = getattr(router, "qb_histogram", None)
            bin_bounds = getattr(router, "qb_bin_bounds", None)
            if histogram is None or bin_bounds is None:
                raise RuntimeError(
                    "Quantile balancing requested, but the router does not expose QB metadata."
                )
            pointer_values.extend((histogram.data_ptr(), bin_bounds.data_ptr()))
            if router_fusion and not te_qb_api:
                raise RuntimeError(
                    "Quantile balancing requested with router fusion, but the installed "
                    "Transformer Engine router does not expose the QB API."
                )
        ...
        pointers[id(router)] = tuple(pointer_values)

    print_rank_0(
        f"[moe_perf] router_bias_metadata method={method}"
        f" router_count={len(routers)} te_qb_api={te_qb_api}"
        f" qb_histogram_mode={'fused_atomic' if use_quantile_balancing and router_fusion else 'none'}"
        f" qb_num_bins={getattr(config, 'moe_router_qb_num_bins', None)}"
    )
    return pointers
...
def _verify_router_bias_pointers(
    ...
    for router in _router_bias_modules(layers):
        expected = expected_pointers[id(router)]
        actual = [router.expert_bias.data_ptr()]
        if len(expected) > 1:
            actual.extend((router.qb_histogram.data_ptr(), router.qb_bin_bounds.data_ptr()))
        if tuple(actual) != expected:
            raise RuntimeError(
                f"Router bias/QB CUDA storage changed: expected={expected}, actual={tuple(actual)}."
            )
...
def _verify_qb_histograms(
    ...
    if _router_bias_update_method(config) != "quantile":
        return
...
def _time_router_bias_finalization(config, layers: torch.nn.ModuleList) -> float:
    update_fn = _router_bias_update_fn(config)
    if update_fn is None:
        return 0.0
    ...
    finalize_model_grads_module.reset_model_temporary_tensors(config, [layers])
    end.record()
    end.synchronize()
    return start.elapsed_time(end)


def _uses_situ_glu(config) -> bool:
    situlu = getattr(activations, "situlu", None)
    return situlu is not None and getattr(config, "activation_func", None) is situlu
...
def _verify_situ_glu_fused_backend(layers: torch.nn.ModuleList) -> None:
    ...
    if expected not in selected:
        raise RuntimeError(
            "SiTU-GLU requested with the Transformer Engine op fuser, but TE did not select "
            f"the fused FC1 -> SiTU-GLU -> FC2 CuTe DSL op; selected={selected}."
        )
...
def _moe_only_flops_per_iteration(args, layer_specs: list[MoELayerSpec]) -> float:
    ...
    return (
        ...
        * _moe_layer_gemm_flops(
            ...
            gated_linear_unit=args.swiglu or getattr(args, "situ_glu", False),
        )
    )
...
def _run_moe_perf(
    ...
    try:
        for iteration in range(total_iters):
            ...
            if (
                iteration == 0
                and _uses_situ_glu(config)
                and getattr(config, "use_transformer_engine_op_fuser", False)
            ):
...
def main() -> None:
    ...
    if args.rank == 0:
        print_rank_0(
            ...
            f"router_bias_update_method={_router_bias_update_method(config)} "
            f"qb_num_bins={getattr(config, 'moe_router_qb_num_bins', None)} "
            f"latent_up_rmsnorm={getattr(config, 'moe_latent_up_projection_rmsnorm', False)} "
            f"situ_glu={_uses_situ_glu(config)}"
        )
